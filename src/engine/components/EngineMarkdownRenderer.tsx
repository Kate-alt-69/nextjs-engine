// ─────────────────────────────────────────────────────────────────────────────
// Engine — universal EngineMarkdown renderer
//
...
export const ENGINE_MARKDOWN_CSS = `
...
export interface EngineMarkdownRendererProps extends MarkdownProps {
	/** Styles already resolved by the server compiler or legacy client resolver. */
	articleStyle?: CSSProperties;
	/** Classes already resolved by the server compiler or legacy client resolver. */
	articleClassName?: string;
	/** Identity already resolved by the owning renderer. */
	articleId?: string;
}

export function EngineMarkdownRenderer({
	content = "",
	textColor = "#30475f",
	headingColor = "#07111f",
	linkColor = "#12304c",
	...
	articleClassName,
	articleId,
}: EngineMarkdownRendererProps) {
	const normalizedContent = normalizeEngineMarkdownSource(content);
	...
	const mergedClass = joinClassNames(articleClassName ?? className, articleAnimationClass);
	const resolvedId = articleId ?? id ?? point;
