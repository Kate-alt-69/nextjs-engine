...
function renderServerNode(
	...
	if (compiled.runtime === "client" || !SERVER_RENDERED_TYPES.has(String(node.type))) {
		return <EngineClientIsland key={compiled.id} node={{ ...node, children: undefined }} config={config} slots={slots} debug={debug}>{children}</EngineClientIsland>;
	}
	...
	switch (node.type) {
		case "box": {
			...
			return wrapHref(props.href, <Tag key={compiled.id} {...debug} id={id} className={className} style={style}>{children}</Tag>);
		}
		case "stack": {
			...
			return wrapHref(props.href, <div key={compiled.id} {...debug} id={id} className={className} style={stackStyle}>{props.dividers === true ? items.flatMap((child, index) => index < items.length - 1 ? [child, <hr key={`${compiled.id}-${index}`} style={{ border: 0, borderTop: "1px solid var(--e-divider, rgba(0,0,0,.1))", margin: 0 }} />] : [child]) : children}</div>);
		}
		case "grid": {
			const columns = props.autoFit === true ? `repeat(auto-fit, minmax(${String(props.minColWidth ?? "200px")}, 1fr))` : props.columns ?? 1;
			return wrapHref(props.href, <div key={compiled.id} {...debug} id={id} className={className} style={compileEngineStyles({ ...props, display: "grid", columns }, collector, props.style as any)}>{children}</div>);
		}
		case "text": {
			...
			return wrapHref(props.href, <Tag key={compiled.id} {...debug} id={id} className={className} style={compileEngineStyles(props as any, collector, { ...derived, ...(props.style as CSSProperties | undefined) })}>{renderTextContent(props, children)}</Tag>);
		}
		case "heading": {
		...
		case "hero": {
			...
			const hero = <section key={compiled.id} {...debug} id={id} className={className} style={outer}>{typeof props.overlay === "string" ? <div aria-hidden="true" style={{ position: "absolute", inset: 0, background: props.overlay, zIndex: 0, pointerEvents: "none" }} /> : null}<div style={inner}>{children}</div></section>;
		...
		case "EngineLink": {
			...
			return external ? <a key={compiled.id} {...debug} id={id} href={href} target={String(props.target ?? "_blank")} rel="noopener noreferrer" className={className} style={style}>{content}</a> : <NextLink key={compiled.id} {...debug} id={id} href={href} className={className} style={style}>{content}</NextLink>;
		}
		case "spacer": {
			const size = props.size ?? "1rem";
			return <div key={compiled.id} {...debug} aria-hidden="true" style={(props.axis === "x" ? { width: size, display: "inline-block" } : { height: size, width: "100%" }) as CSSProperties} />;
		}
		case "divider": {
			const vertical = props.orientation === "vertical";
			return <hr key={compiled.id} {...debug} aria-hidden="true" style={{ border: 0, borderTop: vertical ? undefined : `${String(props.thickness ?? "1px")} ${String(props.style ?? "solid")} ${String(props.color ?? "var(--e-border, #e2e8f0)")}`, borderLeft: vertical ? `${String(props.thickness ?? "1px")} ${String(props.style ?? "solid")} ${String(props.color ?? "var(--e-border, #e2e8f0)")}` : undefined, height: vertical ? "100%" : undefined }} />;
		}
		case "option": return <option key={compiled.id} {...debug} value={String(props.value ?? "")} disabled={props.disabled === true}>{String(props.label ?? node.children ?? props.value ?? "")}</option>;
		case "optgroup": return <optgroup key={compiled.id} {...debug} label={String(props.label ?? "")} disabled={props.disabled === true}>{children}</optgroup>;
		case "label": return <label key={compiled.id} {...debug} id={id} className={className} style={style} htmlFor={String(props.htmlFor ?? props.forInput ?? "") || undefined}>{children}</label>;
		case "slot": {
