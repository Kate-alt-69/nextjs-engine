...
export const roavioTheme: EngineTheme = {
  ...
  globalStyles: `
    *,*::before,*::after{box-sizing:border-box}
    ...
    .rv-toolbar{display:grid;grid-template-columns:minmax(280px,1fr) auto auto;gap:.7rem;align-items:center;width:min(980px,100%);margin:clamp(3rem,9vh,6rem) auto 1.2rem}.rv-input,.rv-select{width:100%;border:1px solid var(--rv-line);background:var(--rv-card);border-radius:14px;padding:.78rem .9rem;color:var(--rv-ink);outline:none}.rv-input:focus,.rv-select:focus{border-color:color-mix(in srgb,var(--rv-green) 60%,transparent);box-shadow:0 0 0 3px color-mix(in srgb,var(--rv-green) 12%,transparent)}
    ...
    @media(hover:hover) and (pointer:fine){.rv-result-card .rv-city-thumb__engine img{transform:scale(1);will-change:transform}.rv-result-card:hover .rv-city-thumb__engine img,.rv-result-card:focus-within .rv-city-thumb__engine img{transform:scale(1.035)}}
    ...
    .rv-footer-theme-slot{display:flex;align-items:center;justify-content:flex-end;min-height:40px;flex:0 0 auto}.rv-theme-toggle{position:static;z-index:auto;border:1px solid var(--rv-line);border-radius:999px;padding:.25rem .62rem .25rem .25rem;background:var(--rv-card-soft);color:var(--rv-ink);box-shadow:none;backdrop-filter:none;display:inline-flex;align-items:center;gap:.35rem;cursor:pointer;font-size:.72rem;font-weight:800;transition:background .2s ease,border-color .2s ease,transform .2s ease}.rv-theme-toggle:hover{border-color:color-mix(in srgb,var(--rv-green) 44%,var(--rv-line));background:color-mix(in srgb,var(--rv-card-soft) 82%,var(--rv-green) 18%)}.rv-theme-toggle:active{transform:scale(.97)}.rv-theme-toggle canvas{display:block}
